...
function DisplaySessionFilters() {
    ...
    if (FiltersApplied != "") {
        $("#lblFilterCriteria").text("Current Filters Applied : " + FiltersApplied);
        $("#lblFilterCriteria2").text("Current Filters Applied : " + FiltersApplied);
    }
}
...
function AddIndiactor_ClosureStatus() {
    ...
    $.each(data, function (i, row) {
        ...
        if (OVERALL_INSTALLATION_STATUS == "Fully Installed") {
            ...
            $('tr[data-uid="' + row.uid + '"] td:nth-child(10)').append('<img name="imgIndiacot" style="float:right !important; height:20px !important;" src="' + bulletGreen + '"/>');
        }
    });
}

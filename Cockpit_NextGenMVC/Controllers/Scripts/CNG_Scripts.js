...
function _LoadWaterfallChart() {
    ...
    if (GridOrders._total != GridOrders.data().length) {
        ...
        var grd_filters = GridOrders.filter();
        var grd_allData = GridOrders.data();
        var grd_query = new kendo.data.Query(grd_allData);
        var data = grd_query.filter(grd_filters).data;
    }
    else {

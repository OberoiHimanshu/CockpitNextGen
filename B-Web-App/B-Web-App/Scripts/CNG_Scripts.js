...
function _LoadWaterfallChart() {
    ...
    if (gridDataSource._total > 0) {
        var totalRows = gridDataSource._total;
        ...
        for (var i = 0; i < totalRows; i++) {
            ...
            if (currentDataItem.EXPECTED_RELEASE_DATE != null && currentDataItem.EXPECTED_RELEASE_DATE.getYear() > 0) {
                ...
                if (ReleaseDate.getMonth() < TodaysDate.getMonth()) {
                    ReleasePriorCurrentMonth += currentDataItem.BACKLOG_AMT;
                } //Release CurrentMonth
                else if (ReleaseDate.getMonth() == TodaysDate.getMonth()) {
        ...
        FutureRelease = FutureMonth1 + FutureMonth2 + FutureMonth3;
        ...
        var WaterfallData = [
        {
            ...
            period: "Past Months",
            amount: ReleasePriorCurrentMonth * -1
        }, {
            period: "No Exp. Rel.",
            amount: NoExpectedReleaseDate 
        }, {
            period: "TBC",
            amount: (NoExpectedReleaseDate * -1) + ReleasePriorCurrentMonth
        }, {
            period: "Future",
            amount: FutureRelease.toString()
        }, {
            period: monthNames[d.getMonth() +1],
            amount: FutureMonth1 *-1
        }, {
            period: monthNames[d.getMonth()+2],
            amount: FutureMonth2 *-1
        }, {
            period: monthNames[d.getMonth()+3],
            amount: FutureMonth3 *-1
        } ];

        $("#WaterfallChart").kendoChart({
            title: {
                text: "Shipped but not Invoiced - Expected Clear Dates based Projection"
            },
            legend: {
                visible: false
            },
...
function _LoadPieChart() {
    ...
    if (gridDataSource._total > 0) {
        ...
        $("#PieChart").kendoChart({
            title: {
                text: "Shipped but not Invoiced - Reasons Distribution"
            },
            dataSource: {
                data: myData
            },
            ...
            seriesColors: ["#03a9f4", "#ff9800", "#fad84a", "#4caf50", "#33a9f4", "#bb9800", "#ccd84a", "#5daf50"],
            tooltip: {
                visible: true,
                template: "#= category # - #= kendo.format('{0:P}', percentage)#"
            }
